...
###### This document implements systematic hyperparameter tuning for MaxEnt models using genetic algorithms (GA) as implemented in SDMtune package. The optimization process explores regularization multipliers (0.2-7.0) and feature class combinations to maximize model performance (AUC) while preventing overfitting through systematic validation procedures.
...
processing and load required packages:
    ...
    ## Ncells  5913288 315.9   10987228 586.8 10987228 586.8
    ## Vcells 12011124  91.7   21469141 163.8 21469141 163.8
...
Control model complexity and overfitting: - Range: 0.2 to 7.0 in steps
...
combinations, resulting in more stable and generalizable predictions.
    ...
    # Display optimized model configuration
    print(best_model)

## Section 11: Model Performance Evaluation

We evaluate final model performance using the validation and test set.
...
<!-- -->
    ...
    roc_plot <- plotROC(best_model, test)
    plot(roc_plot)

![](Tuning_MaxEnt_files/figure-markdown_strict/model-evaluation-1.png)

    auc_value <- auc(best_model, test)
    cat("Area Under Curve (AUC):", auc_value, "\n")

    ## Area Under Curve (AUC): 0.9897212
...
![](Tuning_MaxEnt_files/figure-markdown_strict/jackknife-analysis-1.png)
...
![](Tuning_MaxEnt_files/figure-markdown_strict/correlation-plot-1.png)
